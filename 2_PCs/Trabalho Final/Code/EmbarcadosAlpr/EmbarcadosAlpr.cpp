...
inline void Logger(string logMsg) 
{
	...
	ofstream ofs("/home/pi/alpr/log/alpr.log", std::ios_base::out | std::ios_base::app);
	ofs << now << " - " << logMsg << '\n';
	ofs.close();
}
...
void* ScanPlate(void* arg)
{
	...
	for (; Continuar;)
	{
		...
		if (ValidarPlacas(placas.plates[0].topNPlates))
		{
			...
			{
				sleep(1);
				cout << "Aguardando carro passar" << endl;
			}
			driver(0, PASSOS);
		}
		else
...
bool ValidarPlacas(vector<alpr::AlprPlate>& placas)
{
	...
	for (auto placa : str_placas)
	{
		...
		if (find_if(placa.begin() + 3, placa.end(), [](char c) { return !isdigit(c); }) != placa.end())
		{
			...
			replace(placa.begin() + 3, placa.end(), 'P', '9');
		}

		replace(placa.begin(), placa.begin() + 3, '0', 'O');

		cout << "Placa lida: " << placa << endl;
		if (Embarcados::ExisteCarro(placa))
		{
			Logger("Veiculo autorizado: " + placa);
			return true;
		}

	}
	Logger("Placa nao encontrada: " + str_placas[0]);
	return false;
}
